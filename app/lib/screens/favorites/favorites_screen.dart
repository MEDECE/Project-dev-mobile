...
import 'package:formation_flutter/screens/favorites/favorites_empty.dart';
...
class _FavoritesScreenState extends State<FavoritesScreen> {
  late final FavoritesFetcher _fetcher;

  @override
  void initState() {
    super.initState();
    _fetcher = FavoritesFetcher()..load();
  }
      ...
      child: Scaffold(
        ...
        body: Consumer<FavoritesFetcher>(
          builder: (context, fetcher, _) => switch (fetcher.state) {
            FavoritesFetcherLoading() =>
              const Center(child: CircularProgressIndicator()),
            FavoritesFetcherError() =>
              const Center(child: Text('Erreur de chargement')),
            FavoritesFetcherSuccess(:final barcodes) => barcodes.isEmpty
                ? const FavoritesEmpty()
                : ListView.builder(
                    padding: const EdgeInsets.symmetric(vertical: 8),
                    clipBehavior: Clip.none,
                    itemCount: barcodes.length,
                    itemBuilder: (_, i) => _FavoriteCard(barcode: barcodes[i]),
                  ),
          },
        ),
      ),
...
class _FavoriteCard extends StatelessWidget {
  const _FavoriteCard({required this.barcode});
